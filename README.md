...
- **Due-back times** — every issue names when the asset is expected back, and one that collects a booking takes that time from the booking itself. Anything still out past its time reads as overdue, on the dashboard and at any past instant.
...
| Route | What it's for |
| --- | --- |
| `/` | Dashboard — every asset, its status, holder, due-back time, upcoming reservation and last activity, with issue/return/out-of-service actions and an overdue-only filter |
...
What lands in the store: sixty assets across six kinds, two of which need a certification to hold (harnesses need `HEIGHTS`, gas detectors need `GAS-DETECT`); one asset out of service; twelve workers; roughly two hundred movements; and four bookings.
...
Every request that appends to the ledger or books a window takes a client-generated `idempotencyKey` — the two movement writes, the correction, both service transitions, and reservation creation. The three that don't (cancelling a booking, adding or removing a certification) are idempotent by construction instead; see below. Request and response bodies are validated by Zod schemas in `packages/shared`, so the front end and the API cannot drift apart on a shape.
...
- **Overdue is derived, not stored, and not a status.** Every `ISSUE` movement carries a `dueAt`; an asset is overdue when it is out on an issue whose `dueAt` has passed. Deliberately *not* a fourth `AssetStatus` value: "one holder, ever" is enforced by compare-and-swapping `assets.status` between `IN_STORE` and `ISSUED` (see below), and a status an asset could drift into on its own — with no writer, just the clock — would stop a return from matching that filter and quietly break the invariant. Required, not optional — it was optional until reservations began gating issues, and is not any more: an issue that collects a booking takes its due-back time from that booking's end, and a loan that collects nothing still has to be back before the next booking starts, so both paths need a time to reason about. A blank one would also make "out too long" unanswerable for exactly the loans nobody is tracking. The invariant checker holds the rule from both ends: a due-back time only ever sits on an issue, and every issue has one. The replay carries `dueAt`, so `GET /store?asOf=` reports overdue *as of that instant* rather than as of now.
...
**Idempotency:** every request that appends to the ledger or books a window carries a client-generated `idempotencyKey`, unique-indexed on `movements` and `reservations`. A retry with the same key hits a duplicate-key error, which is interpreted as "already applied" and returns the original result — this is what makes a double-click, a retried request, or a refresh mid-submit land exactly once.

Three endpoints deliberately carry no key, because a second delivery of the same request cannot do damage. Cancelling a booking is a compare-and-set on `status: ACTIVE`, so the retry matches nothing and the first cancel's reason stands. Adding or renewing a certification is an upsert keyed on the code, so replaying it writes the same row it already wrote; removing one is a `$pull` of a code that is already gone. A key on these would buy a stored result to replay, not a guarantee they lack — and none of them writes to the ledger, so there is no history for a duplicate to corrupt.
